package net.argus.emessage.client.gui.config.profile;
...
public class ProfileConfig extends ConfigManager {
	...
	public void load() {
		...
		String[] lines = profile.toArray();
		
		listModel.removeAllElements();
		for(int i = 0; i < lines.length / 2; i++) {
			name = profile.getValue(lines[indexOf(lines, "profile" + i + ".name")]); 
			ip = profile.getValue(lines[indexOf(lines, "profile" + i + ".ip")]);
			
			listModel.addElement(new Profile(name, ip));
		}
	}
	
	private static int indexOf(String[] lines, String key) {
		for(int i = 0; i < lines.length; i++) {
			String li = lines[i];
			if(li.substring(0, li.indexOf('=')).equals(key))
				return i;
		}
