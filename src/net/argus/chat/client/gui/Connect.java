...
import net.argus.chat.client.gui.connect.SplashConnect;
...
public class Connect extends Thread {
	...
	public void run() {
		...
		if(ClientConfig.getPort() != -1) {
			...
			if(host != null) {
				pseudo = JOptionPane.showInputDialog(GUIClient.getFrame(), Lang.get("text.pseudo.name"), "Client", JOptionPane.DEFAULT_OPTION);
				if(pseudo != null) {
					password = JOptionPane.showInputDialog(GUIClient.getFrame(), Lang.get("text.password.name"), "Client", JOptionPane.DEFAULT_OPTION);
					if(password != null) {
	...
	public String getDefaultHost() {
		...
		if(index == -1) {
			int result = OptionPane.showMessageDialog(GUIClient.frame, Lang.get("option.nodefaulthost.name"),
